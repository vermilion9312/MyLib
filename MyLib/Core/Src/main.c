...
int main(void)
{
  ...
  Input*  button_1 = new_Input(BUTTON_1_GPIO_Port, BUTTON_1_Pin);
  Input*  button_2 = new_Input(BUTTON_2_GPIO_Port, BUTTON_2_Pin);
  Input*  button_3 = new_Input(BUTTON_3_GPIO_Port, BUTTON_3_Pin);
  Input*  button_4 = new_Input(BUTTON_4_GPIO_Port, BUTTON_4_Pin);

  Output* left_red    = new_Output(LEFT_RED_GPIO_Port,   LEFT_RED_Pin  );
  Output* left_green  = new_Output(LEFT_GREEN_GPIO_Port, LEFT_GREEN_Pin);
  Output* left_blue   = new_Output(LEFT_BLUE_GPIO_Port,  LEFT_BLUE_Pin );

  Output* right_red   = new_Output(RIGHT_RED_GPIO_Port,   RIGHT_RED_Pin  );
  Output* right_green = new_Output(RIGHT_GREEN_GPIO_Port, RIGHT_GREEN_Pin);
  Output* right_blue  = new_Output(RIGHT_BLUE_GPIO_Port,  RIGHT_BLUE_Pin );

  while (1)
  {
	  button_1->update(button_1, NOMAL_OPEN);

	  if (button_1->on_rising_edge(button_1))
	  {
		  left_red->toggle(left_red);
	  }
//	  else
//	  {
//		  left_red->turn_off(left_red);
//	  }
    /* USER CODE END WHILE */
